	...
	private double[][] ApplyConstraints(double[][] chromosomes){
		...
		for(int i=0;i<numberofpopulation;i++){
			for(int j=0;j<chromosomelength;j++){
				...
				result[i][1] = Math.floor(chromosomes[i][1]*Integer.MAX_VALUE) % 30;
				result[i][2] = Math.floor(chromosomes[i][2]*Integer.MAX_VALUE) % 30;
				result[i][3] = Math.floor(chromosomes[i][3]*Integer.MAX_VALUE) % 30;
			}
		}
		return result;
	}
